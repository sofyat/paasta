...
is scheduled by Marthon (on Mesos), Kubernetes, Tron, or ``paasta remote-run``.
Most commonly, it is used to set topological restrictions on tasks.
...
These options are only applicable to tasks scheduled on Kubernetes.
  ...
  * ``node_selectors``: A map of labels a node is required to have for a task
    ...
    * Requires that a node have a priority greater than 1 and less than 5::
...
instance MAY have:
  ...
  * Only ``pool`` from `Placement Options`_.
...
instance MAY have:
  ...
  * Anything in the `Placement Options`_.
